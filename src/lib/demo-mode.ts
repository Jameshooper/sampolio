 ...
 * because those go through `formatCurrency` too.
 *
 ...
 * flag DURING RENDER (before returning JSX) from `demoMode`, so the very first
 * paint after a toggle already formats with the correct setting (an effect
 * would be one paint late). The write is idempotent, so repeated render-phase
 * calls under StrictMode / concurrent re-renders are harmless.
 *
