...
- `bank-ledger-table.tsx` — Imported transaction ledger for one linked account: search box, infinite-scroll desktop DataTable (rows revealed as the inner scroll area nears its end) with a "Split" flag-pill column, and compact month-grouped mobile rows (status dot, expand chevron, sibling "already split" pill + Split-this button). Rendered on the `/bank` page.
...
- `skeletons.tsx` — content-shaped page skeletons (`KpiGridSkeleton`, `ChartsPageSkeleton`, `ListPageSkeleton`, `HomeGlanceSkeleton`/`HomeFeedSkeleton` (Home's two regions, above/below the always-mounted Quick-add card), `SplitDetailSkeleton`) used in place of full-page spinners on heavy pages (instant-shell pattern); size each block to the real content so the swap never shifts layout
