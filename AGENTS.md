...
│   └── page.tsx                           # Home dashboard at / (bank-account glance strip + projected-month tile, merged split balances+activity card; add via the global FAB) — auth-guarded, wraps AppLayout
├── components/
...
│   │   ├── bank.ts                        # Enable Banking: connect/reconnect/refresh, link config, card billing, Home balance glance (getHomeBankGlance)
...
│   │   ├── link-identity.ts               # Pure cross-session/user account identity + freshness (linkIdentityKey, linksShareIdentity, isLinkFresh)
│   │   ├── consent-validity.ts            # Pure per-bank consent-validity request math (computeConsentValidUntil)
│   │   ├── aspsp-info.ts                  # Cached GET /aspsps capability lookups (maximum_consent_validity, required_psu_headers)
...
│   ├── treemap-layout.ts                  # Pure slice-and-dice treemap layout (computeTreemapLayout → % rects; unit-tested) for the CSS mini-treemaps
...
│   ├── demo-mode.ts                       # UI-only money-masking flag (setDemoMask/isDemoMasked, maskMoney); zero imports
...
- **Re-consent matching is hash-set intersection (`identificationHashes` ∪ `identificationHash`) → `accountUid` → IBAN** (`reconcileLinks`): EB rotates account uids every session and hashes every identification basis (IBAN, BBAN, …), so which one is *primary* can change — a non-empty intersection (`linkIdentityHashes`) is the only key that reliably survives for no-IBAN accounts (cards); never drop it from the matcher. A match merges the two hash sets (union). `sync.ts` backfills missing hashes onto pre-existing links via one conditional `GET /sessions/{id}` (an EB-side lookup, no bank allowance spent) that converges (`identificationHashes = [identificationHash]` when the session serves no array), and reads that response's session `status` through `terminalSessionStatus` to stop a dead consent without an extra call.
- **Cross-user fan-out for shared/joint accounts** (one fetch per underlying account per cycle): `linksShareIdentity` (`link-identity.ts`: hash-set intersection, falling back to `linkIdentityKey` equality when either link has no hashes) is the *only* thing that may match links across users. Sibling writes go through the same `mergeTransactions`/`applyLinkBalances`/`autoAnchorAccount` paths (never bypass dedup), never touch a sibling's `backfilledThrough` (their deep backfill needs their own session), and are fully isolated — a fan-out failure never fails the primary sync. Scheduled runs skip-fetch a fresh link (`isLinkFresh`; `skippedFresh` in the audit); manual refreshes never skip (attended calls are exempt from the ASPSP limit). The scheduler's daily budget (`MAX_SCHEDULED_FETCHES_PER_DAY = 3`, ~8h cadence) stays keyed by the single-valued `linkIdentityKey` — shared across users, stable for existing data — and only actually-fetched accounts draw from it.
- **Consent validity is per bank**: `beginConnection`/`beginReconnect` request the smaller of the ASPSP's `maximum_consent_validity` (from `GET /aspsps`, memory-cached) and `CONSENT_MAX_REQUESTED_VALIDITY_DAYS = 365d` via the pure `computeConsentValidUntil`, falling back to `CONSENT_REQUESTED_VALIDITY_DAYS = 180d` when the (cached, failure-tolerant) lookup yields nothing; the bank's returned `access.valid_until` stays authoritative. The observed ceiling is stored as `aspspMaxConsentValiditySeconds` for display only.
- **Attended PSU headers travel as a pair**: the client sends `Psu-Ip-Address` + `Psu-User-Agent` only when an IP is present, never a user agent alone — unattended (scheduler) runs send neither, which keeps "attended ⇔ `psuIp` present" the single switch behind `psuPresent`/`shouldFetchPending`. `required_psu_headers` is deliberately not stored or enforced.
...
- **List-page aggregates** (`split/page.tsx`): an "Across all groups" summary card and an always-expanded **Insights** section, both from the pure engine `src/lib/split-insights.ts`. `aggregatePairwiseNets` sums the viewer's per-counterparty position (per group's `suggestSettleUp`, so it always agrees with each group's settle-up screen; cross-currency kept separate, "(mixed currencies)" note). `getSplitInsights(monthsBack = 12)` (`split-groups.ts`; cached summaries → overlapping months' chunks → pure `computeSplitInsights`) feeds the lazy ECharts `split-spend-chart` (stacked spend with a **By member (default) / By category / By group** toggle — category mode buckets via `bucketSpendByCategory(insights, 8)` top-N + 'Other' and colors via `getCategoryColor`) + `split-net-chart` (running viewer-net line over green/orange **monthly-change bars**; the first bar is measured against `insights.viewerNetBaseline`, and when a large standing balance dwarfs the deltas the bars move to a hidden second y-axis whose zero is pixel-aligned with the primary axis's zero). Both charts' plain-words descriptions (`describeSplitSpend`/`describeSplitNet` in `chart-descriptions.ts`) cover the whole window AND the current month — "this month so far", a typical-month comparison (avg over nonzero prior months, emitted at ≥2), a mode-aware leader-shift sentence, and the net chart's current-month move. **Imported-row paid attribution is a documented LOWER BOUND**: native rows use exact `paidBy` shares, but net-only imported rows credit each member `max(0, net)`.
- **Group-detail "Last 30 days" card** (`src/components/split/group-period-card.tsx`, between the balance banner and the recurring-rules card on `/split/[id]`): pure `computeGroupPeriodInsights(members, rows, fromDate, toDate)` over the page's already-loaded expense chunks (the 30-day window spans ≤2 calendar months, always within the initial 3-chunk load — no extra fetch). Total spend headline + a default-**open** `.collapse-grid` expander holding two small **CSS treemaps** — "By category" (top 5 + an 'Other' bucket, tinted `getCategoryColor`) and "Who paid" (tinted `getAvatarColor`) — laid out by the pure slice-and-dice `computeTreemapLayout` (`src/lib/treemap-layout.ts`, unit-tested; each treemap is `role="img"` whose label carries names + percentages only, money lives in the legends/`title` tooltips), plus top-3 expenses and `describeGroupPeriod` sentences. Same imported-row lower-bound rule (flagged with a caveat note); payments count only toward `settledCents`. Deliberately **no ECharts import** so the detail bundle stays lean; hidden when the group has no expenses at all.
